...
(the next number — `docs/plans/README.md`), a decision in `docs/adr/`, a written
contract in `docs/specs/`. Plan-mode writes to `~/.claude/plans/` — that file is scratch, not
...
- `add-persona` — new coach persona row, voice direction, banned phrases, test
