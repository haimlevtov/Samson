 ...
 *
 * AI-NOTE: the same address is hard-coded in `reset_demo_account()`. Two copies
 *          of one fact, held together by `tests/db/demo-reset.test.ts`, which
 *          calls the function as a user who is not it and expects a refusal.
 */
 ...
 *
 * The SETTINGS stay — timezone, theme, humour ceiling, leaderboard opt-out. They
 * are preferences about using the app rather than training data, and a reset
 * that silently moved somebody's timezone would be doing something nobody asked
 * for.
 *
 ...
 */
export const RESET_KEEPS = ['your sign-in', 'your settings', "the app's own usage log"] as const;
...
export async function resetDemoData(db: Db): Promise<void> {
  ...
  if (error) throw new Error(`resetting the demo account: ${error.message}`);
}
