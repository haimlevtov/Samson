...
--
-- FOUND IN REVIEW of PR #42. Three of the legs tree's five rungs were chosen by
-- name, and the names mislead: `chair-squat` is a Smith-machine squat tagged
-- `machine`, `split-squats` is a jumping split filed under `stretching`, and
-- `smith-machine-pistol-squat` is a Smith-machine split stance. The picker
-- (`availableExercises`) offers neither `machine` to a bodyweight user nor
-- `stretching` to anyone, so the tree stopped at its root for every bodyweight
-- user and its top rung could never open at all. 20260908120000 checked that
-- each slug existed; that was the wrong question.
--
...
--   * no user row can exist — 20260908120100 dropped the write policy;
