...
-- `btrim(E'\t')` and `btrim(U&'\00a0')` are both non-empty and passed the old
-- guard. A tab or a non-breaking space took a numbered slot on everybody's
-- board and rendered as a blank row, which falsifies ADR 0016 §3: appearing is
-- supposed to require having chosen a name to appear under.
--
-- The settings form closed this on its own path — JavaScript's `.trim()` covers
-- both — but the form is not the boundary, and the direct PATCH above skips it.
--
...
--          includes U+00A0 is not portable. Enumerated, it is the same set on
--          every machine that runs this.
create or replace function public.has_visible_name(value text)
returns boolean
language sql
immutable
-- INVARIANT: pinned search_path — schema-invariants.test.ts asserts every
--            function in public has one.
set search_path = ''
as $$
  select btrim(
    coalesce(value, ''),
    -- chr() rather than E'' or U&'' escapes: one unambiguous spelling that
    -- does not depend on standard_conforming_strings, or on the reader knowing
    -- which escape syntax is in play.
    ' '
      || chr(9) -- tab
      || chr(10) -- newline
...
where public.has_visible_name(u.display_name)
