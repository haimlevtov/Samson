...
**Finishing lands on a receipt before History** — the Quest Log redesign, PR 4. `/history/[id]/kept` shows what the session earned, the week against its cap, the streak and any quest it moved, all read from what `award_session_xp` just wrote; **Done** goes on to History carrying the same `?unlocked=`, so a badge still fires there. It is a receipt for this week only: a session from an earlier week goes to its own page instead, because the figures beside it are read as of today.
...
| **A session finished**                     | The receipt: "+N XP" earned by this session's own ledger rows, "Third session this week", the week against its cap in ten segments, the level, the streak, and each accepted quest with "pays on the next weekly run". Done and Review the session.                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **A session that earned nothing**          | "+0", and a reason only when it is known: the week's cap reached. Otherwise "No XP is recorded for this session yet" — the award can fail silently by design, and the page does not invent why.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
