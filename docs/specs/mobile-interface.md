...
| **A badge fires**                          | A sheet over History: the badge in its metal, its name, description and source hint, then All badges (to its card on /badges) and Continue. It is a dialog — focus lands on Continue and stays inside; Escape, the backdrop and Continue close it, and closing drops ?unlocked= so a reload does not fire it twice. A slug the reader does not hold shows nothing. |
