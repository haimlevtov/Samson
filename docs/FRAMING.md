...
- **Everything a coach is** — five character descriptions, their banned-phrase lists, intensities, humour tiers and voice allocation, and since 2026-09-11 a sample line each (migration `20260911130000`). Written by one author and judged "recognisably its own" by the same one; the persona drift eval that would measure any of it does not exist (`docs/PLAN.md` phase 3).
