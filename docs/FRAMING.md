...
| Default LLM budget is $0.50 per user per week                          | `users` column **and** `guard_llm_budget` | **Load-bearing** — a low cap silently truncates phase 2 evals; the figure is in two places since ADR 0026. The sign-in page's default account has $2.00 (ADR 0026's amendment) |
