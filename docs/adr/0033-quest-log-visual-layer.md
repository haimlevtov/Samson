...
about difficulty that the tier did not already claim, and the page says so. A
test holds the map to exactly the eight tiers `achievements_tier_check` allows, so
a ninth tier fails a build rather than rendering unstyled.
...
- **No new arithmetic.** A level, a meter's fill, a count of segments: each comes
  from `src/gamification/` or `src/metrics/`, or from a pure helper in `src/ui/`
  that only divides a figure those modules produced into segments — tested, and
  never a new rule about XP.
...
- Headings change on every page at once, including pages the redesign does not
  ...
  `/evidence`). That is intended: one display face is the point.
...
- Hub's order changes — the board first — which supersedes the ordering argument
  in the Hub page's comments and `docs/specs/mobile-interface.md` §2, and nothing
  else about ADR 0016.
