...
| [ADR 0003](../adr/0003-grants-and-rls.md) | Three foreign keys that no write policy checks: `exercise_equipment.exercise_id` and `.equipment_tag_id`, whose primary key has no `user_id`, so one user's link occupies the pair for everybody — a key change, so a local stack for the types — and `user_equipment.equipment_tag_id`, an existence oracle only. Pinned in `tests/db/schema-invariants.test.ts`, so a fourth fails CI |
