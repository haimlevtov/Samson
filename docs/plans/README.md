...
**Blocked on an OpenRouter key, not on work** — seven of the seventeen open items
below, and worth
...
| [phase-6.md](phase-6.md)                               | The browser pass at 375×812 was not run on PRs 2, 4 or 5. Review found a width bug and a colour-only state in that markup, so it is the check that catches exactly this. Nor on the rework plan's PR 6, the persona preview on Coach, which needs a signed-in session to reach                                                                                                                                                                                                                       |
